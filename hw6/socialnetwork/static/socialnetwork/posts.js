...
function updatePostsAndComments(posts) {
    ...
    // Instead, "prepend" each NEW post to #stream by ascending timestamp (~= Append by descending timestamp)
    $(posts).each(function() {
        // NEW post: a post whose id does NOT exist in the DOM tree (checked by jQuery object length)
    ...
    // For each post, "append" each of its NEW comments to #post-id by ascending timestamp
    $(posts).each(function() {
        // Back up the post variable to be referenced in comments
        var post_id = "#post-" + this.post_id;
