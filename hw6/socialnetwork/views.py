...
def register(request):
    ...
    # Logs in the new user and redirects to the home page (global stream)
    new_user = authenticate(username=form.cleaned_data['username'],
                            password=form.cleaned_data['password1'])
    login(request, new_user)

    messages.success(request, 'Registration succeeded. Welcome to 15-637 Social Network')
    # Redirect by a reverse URL or a view function
...
def edit(request, user_id):
    try:
        ...
        # POST method: process EditUserForm and EditProfileForm
...
def get_picture(request, user_id):
    ...
    if not profile.picture:
        ...
        return HttpResponse(content=open('socialnetwork/static/socialnetwork/default_user.svg', 'rb').read(),
